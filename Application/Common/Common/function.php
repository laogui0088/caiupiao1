...
function xyft_format($caiji){
	// 幸運飛艇期號應該是10位數，如果數據庫存儲的是9位(YYMMDDXXX)，則補齊為10位(20YYMMDDXXX)
	$periodNumber = $caiji['periodnumber'];
	// 如果期號是9位數，補齊為10位
	if (strlen($periodNumber) == 9) {
		$periodNumber = '20' . $periodNumber;
	}
...
function ssc_format($caiji){
	...
	$periodNumber = $caiji['periodnumber'];
	// 如果期號長度大於5位，只取後5位
	if (strlen($periodNumber) > 5) {
		$periodNumber = substr($periodNumber, -5);
	}
	...
	if (strtotime($awardTime) > strtotime("00:00:00") && strtotime($awardTime) < strtotime("00:03:00")) {
		$next_periodNumber = '00001';
	} else {
		// 下一期也保持5位數格式
		$full_next = $caiji['periodnumber'] + 1;
		$next_periodNumber = strlen($full_next) > 5 ? substr($full_next, -5) : str_pad($full_next, 5, '0', STR_PAD_LEFT);
	}
