...
class FenController extends BaseController{
	...
	public function addpage2(){
		if (IS_POST) {
			...
			if (!I('post.money')) {
				$this->error('充值金额不能为空！');die;
			}
			if (!I('post.userpay')) {
				$this->error('交易哈希不能为空！');die;
			}
			
			// 验证交易哈希格式（以太坊/TRON格式）
			$txHash = trim(I('post.userpay'));
			if (!preg_match('/^(0x)?[a-fA-F0-9]{64}$/', $txHash)) {
				$this->error('交易哈希格式不正确！请输入完整的64位哈希值');die;
			}
			
			// 检查是否重复提交
			$userid = session('user');
			$exists = M('fenadd')->where(array(
				'type' => 'USDT',
				'userpay' => $txHash
			))->find();
			if($exists){
				$this->error('此交易哈希已提交过，请勿重复提交！');die;
			}

			$user = M('user')->where(array('id' => $userid['id']))->find();
			
			// 获取USDT汇率配置（如果没有配置则默认1:6.5）
			$usdt_rate = C('usdt_rate') ? C('usdt_rate') : 6.5;
			$usdt_amount = floatval(I('post.money'));
			$cny_amount = $usdt_amount * $usdt_rate; // 转换为人民币
			...
			M('fenadd')->add(array(
					...
					'headimgurl' => $user['headimgurl'],
					'type'       => 'USDT',
					'money'      => $cny_amount,  // 存储人民币金额
					'balance'    => $user['points'],
					'username'   => 'USDT充值',
					'userpay'    => $txHash, // 存储交易哈希
					'remark'     => "USDT: {$usdt_amount} (汇率:{$usdt_rate})", // 备注原始USDT金额
					'addtime'    => time(),
			));

			$this->success('USDT充值申请已提交！请等待审核，审核通过后积分将自动到账');die;
		} else {
			// 获取USDT钱包地址
			$userid = session('user');
			if (C('agent_pay') == '1') {
				...
				// 优先读取专用USDT配置
				$usdt_config = M('config')->where("id = 5")->find();
				if($usdt_config && $usdt_config['kefu']){
					$pay['usdt_address'] = $usdt_config['kefu'];
				}else{
					// 如果没有id=5的配置，使用id=2（向下兼容）
					$info = M('config')->where("id = 2")->find();
					$pay['usdt_address'] = $info['kefu'];
				}
			}
			
			// 获取USDT配置信息
			$pay['usdt_network'] = C('usdt_network') ? C('usdt_network') : 'TRC20'; // 默认TRC20
			$pay['usdt_rate'] = C('usdt_rate') ? C('usdt_rate') : 6.5; // 汇率
			$pay['usdt_min'] = C('usdt_min') ? C('usdt_min') : 10; // 最小充值
			$pay['usdt_max'] = C('usdt_max') ? C('usdt_max') : 50000; // 最大充值
