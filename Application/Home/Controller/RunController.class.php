...
class RunController extends BaseController
{
    ...
    public function xyft()
    {
        ...
        // 10期结果
        $list = M('number')->distinct(true)->field('id',true)->where("game='xyft'")
            ->order("id DESC")
            ->limit(10)
            ->select();
...
//new beijing
    ...
    public function trend()
    {
        ...
        $list1 = M('number')->where("game='xyft'")->distinct(true)->field('id',true)
            ->order("id DESC")
            ->limit(1)
            ->select();
        $list2 = M('number')->where("game='ssc'")->distinct(true)->field('id',true)
            ->order("id DESC")
            ->limit(1)
            ->select();
    ...
    {
        $list = M('number')->where(array('game' => I('game', 'xyft'), 'time' => array('between',array(strtotime(date('Y-m-d') . ' 00:00:00'), strtotime(date('Y-m-d') . ' 23:59:59')))))->distinct(true)->field('id',true)
            ->order("id DESC")
            ->select();
        foreach ($list as $key => $value) {
            $current_number = $value;
